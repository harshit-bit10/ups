...
async def upscale_image_enhanced(img_path: Path) -> Path:
    try:
        ...
        upscaled = await asyncio.to_thread(
            cv2.bilateralFilter, upscaled, 10, 120, 120
        )
        ...
        async def enhance_image(image: Image.Image) -> Image.Image:
            image = await asyncio.to_thread(ImageEnhance.Sharpness(image).enhance, 23.5)  # More sharpness
            image = await asyncio.to_thread(ImageEnhance.Contrast(image).enhance, 1.0)  # Slight contrast boost
            return image
